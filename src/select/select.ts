  ...
  template: `
        <div tabindex="0"
            ...
            <ul *ngIf="optionsOpened && options && options.length > 0 && !firstItemHasChildren"
                ...
                <li *ngFor="let o of options" role="menuitem" [class]="o.classNames || null">
                    <div class="ui-select-choices-row"
                         [class.active]="isActive(o)"
                         (mouseenter)="selectActive(o)"
                         (click)="selectMatch(o, $event)">
                        <a href="javascript:void(0)" class="dropdown-item">
                            <div [innerHtml]="sanitize(o.html | highlight:inputValue)"></div>
                        </a>
                    </div>
                </li>
            </ul>
        ...
        <span *ngFor="let a of active">
            <span class="ui-select-match-item btn btn-default btn-secondary btn-xs"
               ...
               <span [innerHtml]="sanitize(a.html)"></span>
           </span>
        </span>
    </span>
            <input type="text"
...
export class SelectComponent implements OnInit, ControlValueAccessor {
  ...
  @Input() public htmlField: string = 'html';
  @Input() public classNames: string = 'classNames';
  ...
  public set items(value: Array<any>) {
    if (!value) {
      ...
      this.itemObjects = this._items.map((item: any) => (typeof item === 'string' ? new SelectItem(item) : new SelectItem({
        ...
        classNames: item[this.classNames],
  ...
  public set active(selectedItems: Array<any>) {
    if (!selectedItems || selectedItems.length === 0) {
      ...
      if (this.multiple === false && this._active.length > 0) {
        this.inputValue = this._active[0].text;
      }
    }
  }
